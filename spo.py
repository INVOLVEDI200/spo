حتماً. این نسخه **کامل، یک‌پارچه و آماده اجرا** است؛ تو فقط مقادیر Variables، لینک‌ها، آیدی کاستوم ایموجی‌ها و لوگوی PUBG رو عوض کن.

این کد دقیقاً برای حالت رفیقت نوشته شده:

- پست مبدا: **عکس + کپشن**
- داخل کپشن: متن آبی مثل **لینک کامل ویدیو**
- کد لینک مخفی رو از `entities` می‌خونه
- میره پیام ویدیویی رو از کانال دوم پیدا می‌کنه
- ویدیو رو با کپشن جدید PUBG توی کانال مقصد می‌فرسته
- فرم کپشن دقیقاً طبق فرم رفیقت ساخته شده
- لینک‌های آبی مثل `PUBG_BUY_SELL`، `Group`، `MM`، `Website`، `BUY NOW`، `SELL YOUR ACCOUNT` تعریف شده‌اند
- Quote برای هشتگ‌ها و دکمه‌های آخر مثل کد خودت فعال است
...
# شمارنده اکانت
ACCOUNT_COUNTER_START = get_env_int("ACCOUNT_COUNTER_START", 1)
...
WEBSITE_URL = get_env_value("WEBSITE_URL", "https://example.com/")
...
PUBG_BUY_SELL_URL = get_env_value("PUBG_BUY_SELL_URL", "https://t.me/PUBG_BUY_SELL")
...
PUBG_LOGO_CUSTOM_EMOJI_IDS = [
    get_env_int("PUBG_LOGO_01", 0),
    get_env_int("PUBG_LOGO_02", 0),
    get_env_int("PUBG_LOGO_03", 0),
    get_env_int("PUBG_LOGO_04", 0),
    get_env_int("PUBG_LOGO_05", 0),
    get_env_int("PUBG_LOGO_06", 0),

    get_env_int("PUBG_LOGO_07", 0),
    get_env_int("PUBG_LOGO_08", 0),
    get_env_int("PUBG_LOGO_09", 0),
    get_env_int("PUBG_LOGO_10", 0),
    get_env_int("PUBG_LOGO_11", 0),
    get_env_int("PUBG_LOGO_12", 0),
]
